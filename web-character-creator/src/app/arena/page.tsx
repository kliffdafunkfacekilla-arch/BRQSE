...
export default function Arena() {
    ...
    useEffect(() => {
        fetch('/data/last_battle_replay.json')
            .then(res => res.json())
            .then(data => {
                // Init positions (Demo logic)
                // We initialize simulation to the saved state? Or reset?
                // The data packet has 'combatants' with max_hp.
                // We set their initial positions based on team or data.
                // The generator puts them at (2,5) and (7,5).
    ...
    return (
        <div className="min-h-screen bg-[#050510] text-white p-4 flex flex-col items-center justify-center">
            <h1 className="text-3xl font-bold title-gradient mb-4 tracking-widest">COMBAT SIMULATION CLIENT</h1>

            {/* THE VIEWPORT */}
            <div
                className="relative bg-grid-pattern border border-[#00f2ff]/30 shadow-[0_0_50px_rgba(0,242,255,0.15)] rounded-lg overflow-hidden"
                style={{
            ...
            {/* HUD CONTROLS */}
            <div className="mt-6 glass-panel p-4 w-[500px] flex justify-between items-center border border-[#ffffff10]">
                <div className="font-mono text-[#00f2ff] text-xs">EVENT: {step} / {log.length}</div>
                <div className="space-x-4">
                    <button
                        onClick={() => setPlaying(!playing)}
                        className={`px-6 py-2 rounded font-bold transition-all ${playing ? 'bg-red-500/20 text-red-400 border border-red-500' : 'bg-green-500/20 text-green-400 border border-green-500'}`}
                    >
                        {playing ? "PAUSE FEED" : "EXECUTE"}
                    </button>
            ...
            {/* SYSTEM LOG */}
            <div className="mt-4 w-[500px] font-mono text-[10px] text-gray-500 space-y-1 p-2 bg-black/40 rounded border border-gray-800">
                {consoleMsg.map((msg, i) => (
                    <div key={i} className="opacity-80">{msg}</div>
                ))}
            </div>

            <Link href="/" className="mt-8 text-gray-600 hover:text-[#00f2ff] text-xs tracking-widest">← TERMINATE SESSION</Link>
        </div>
    );
}
