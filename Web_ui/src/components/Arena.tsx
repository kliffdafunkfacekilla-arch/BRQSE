...
export default function Arena({ onStatsUpdate }: ArenaProps) {
    ...
    const processEvent = (event: ReplayEvent) => {
        if (!event) return;
        setConsoleMsg(prev => [`> ${event.actor}: ${event.type.toUpperCase()} ${event.damage ? `(${event.damage} DMG)` : ''}`, ...prev].slice(0, 3));
    ...
    // --- THE FX ENGINE ---
    const triggerEffect = (x: number, y: number, style: string) => {
        const id = Date.now();
        ...
        if (style.includes('crit')) img = '/effect/blood_splatter.png'; // Make sure you have one!

        setVisuals(prev => [...prev, { id, x, y, img }]);
        ...
        <div className="flex flex-col items-center w-full max-w-2xl mx-auto">
            ...
            >
                ...
                {combatants.map((c, i) => (
                    ...
                    >
                        ...
                        <div className="w-full max-w-[40px] h-1 bg-stone-900 mt-1 rounded-full overflow-hidden border border-white/10 relative -top-1">
                            {/* Adjusted color for Red team to match user preference */}
                ...
                {visuals.map((v) => (
                    <div
                        key={v.id}
                        className="absolute z-20 pointer-events-none flex items-center justify-center animate-ping-slow" // You can add custom CSS animation
                        style={{
                            width: `${100 / GRID_SIZE}%`, height: `${100 / GRID_SIZE}%`,
                            left: `${v.x * (100 / GRID_SIZE)}%`, top: `${v.y * (100 / GRID_SIZE)}%`,
                            transition: 'none' // FX snap instantly
                        }}
                    >
            ...
            {/* ... Footer ... */}
            <div className="w-full bg-black border border-stone-800 border-t-0 p-2 h-8 flex items-center rounded-b-lg">
                <span className="text-[10px] font-mono text-stone-600 mr-2">&gt;</span>
                <span className="text-[10px] font-mono text-[#00f2ff] animate-pulse">{consoleMsg[0]}</span>
            </div>
        </div>
    );
}
