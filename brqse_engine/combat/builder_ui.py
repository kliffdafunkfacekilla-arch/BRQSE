        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                ...
                if self.rect.collidepoint(event.pos):
                    # If it was open, we already checked above. 
                    # If we clicked button while open, we want to close it.
                    # Current logic: if open, 'elif' skipped -> Stays open.
                    # FIX: Change elif to if, and handle logic carefully.
                    
                    # Simple Toggle Logic:
                    # If Open and Clicked Button -> Close
                    # If Closed and Clicked Button -> Open
                    
                    # My previous logic:
                    # if open: check list. if not list, check outside.
                    # elif rect: toggle.
                    
                    # Fix:
                    if self.is_open:
                         # We checked list above. If we are here, we didn't click list.
                         # check button
                         if self.rect.collidepoint(event.pos):
                             self.is_open = False
                             print("DEBUG: Closed by button toggle")
                             return True
                    else:
                         if self.rect.collidepoint(event.pos):
                             self.is_open = True
